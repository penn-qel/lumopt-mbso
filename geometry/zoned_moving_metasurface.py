...
class ZonedMovingMetasurface3D(MovingMetasurface3D):
    ...
    def __init__(self, posx, posy, rx, ry, min_feature_size, z, h, eps_in, eps_out, inner = 0, outer = np.inf, phi = None, pillars_rotate = True, height_precision = 10, angle_precision = 20, scaling_factor = 1, phi_scaling = 1/180, limit_nearest_neighbor_cons = True, make_meshgrid = False, dx = 10e-9, params_debug = False):
        self.active = np.full(posx.shape, True)
    ...
    def set_active_pillars(self, inner, outer):
        ...
        self.inner = inner
        self.outer = outer
        dist = np.sqrt(np.square(self.init_x + self.offset_x) + np.square(self.init_y + self.offset_y))
        self.active = np.logical_and(dist >= inner, dist < outer)
    ...
    def get_from_params(self, params):
        ...
        offset_x[self.active] = offset_x_active
        offset_y[self.active] = offset_y_active
        rx[self.active] = rx_active
        ry[self.active] = ry_active
        phi[self.active] = phi_active
    ...
    def get_current_params(self):
        ...
        s2 = self.phi_scaling
        return np.concatenate((self.offset_x[self.active]*s1, self.offset_y[self.active]*s1, self.rx[self.active]*s1, self.ry[self.active]*s1, self.phi[self.active]*s2))
    ...
    def calculate_bounds(self):
        ...
        radius_bounds = [(self.min_feature_size*self.scaling_factor/2, np.inf)]*self.rx[self.active].size*2
        offset_bounds = [(-np.inf, np.inf)]*self.rx[self.active].size*2
        phi_bounds = [(-np.inf, np.inf)]*(self.rx[self.active].size)
    ...
    def calculate_gradients(self, gradient_fields):
        ...
        total_deriv = super().calculate_gradients(gradient_fields)
        deriv_x, deriv_y, deriv_rx, deriv_ry, deriv_phi = np.split(total_deriv, 5)

        #Reconcatenates with only active portion
        return np.concatenate((deriv_x[self.active,:], deriv_y[self.active,:], deriv_rx[self.active,:], deriv_ry[self.active,:], deriv_phi[self.active,:]))
