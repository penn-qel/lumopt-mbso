...
def interpolate_Efield(x, y, z, wl, fields):
    ...
    yi = np.searchsorted(fields.y, y).reshape(x.size, 1, 1)
    zi = np.searchsorted(fields.z, z).reshape(x.size, 1, 1)
    wli = np.searchsorted(fields.wl, wl).reshape(x.size, 1, 1)
    x = x.reshape(x.size, 1, 1)
    y = y.reshape(x.size, 1, 1)
    z = z.reshape(x.size, 1, 1)
    wl = wl.reshape(x.size, 1, 1)
    ...
    E00 = fields.E[xi-1, yi-1, zi-1,wli,:].squeeze()*(1 - xd) + fields.E[xi, yi-1, zi-1,wli,:].squeeze()*xd
    E01 = fields.E[xi-1, yi-1, zi,wli,:].squeeze()*(1-xd) + fields.E[xi, yi-1, zi,wli,:].squeeze()*xd
    E10 = fields.E[xi-1, yi, zi-1,wli,:].squeeze()*(1-xd) + fields.E[xi, yi, zi-1,wli,:].squeeze()*xd
    E11 = fields.E[xi-1, yi, zi,wli,:].squeeze()*(1-xd) + fields.E[xi, yi, zi,wli,:].squeeze()*xd
...
def interpolate_Hfield(x, y, z, wl, fields):
    #takes in a fields structure, and (N,) arrays of x,y,z,wl points to interpolate on. Returns H only
    ...
    H00 = fields.H[xi-1, yi-1, zi-1,wli,:].squeeze()*(1 - xd) + fields.H[xi, yi-1, zi-1,wli,:].squeeze()*xd
    H01 = fields.H[xi-1, yi-1, zi,wli,:].squeeze()*(1-xd) + fields.H[xi, yi-1, zi,wli,:].squeeze()*xd
    H10 = fields.H[xi-1, yi, zi-1,wli,:].squeeze()*(1-xd) + fields.H[xi, yi, zi-1,wli,:].squeeze()*xd
    H11 = fields.H[xi-1, yi, zi,wli,:].squeeze()*(1-xd) + fields.H[xi, yi, zi,wli,:].squeeze()*xd
